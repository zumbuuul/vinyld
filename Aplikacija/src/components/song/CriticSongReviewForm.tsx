...
import {
  deleteCriticSongReview,
  saveCriticSongReview,
...
import { criticSongReviewInputSchema } from "@/features/album/review.schemas";

export function CriticSongReviewForm({
  songId,
  songSpotifyId,
  initialTitle,
  initialRating10,
  ...
  const handleSubmit = () => {
    ...
    const parsed = criticSongReviewInputSchema.safeParse({
      songId,
      songSpotifyId,
      title,
      rating10,
    ...
    startTransition(async () => {
      try {
        const result = await saveCriticSongReview(parsed.data);
  ...
  return (
    <FormShell
      title="Nova Kritika"
      description="Napisi profesionalni osvrt na pesmu."
    ...
    >
      ...
      >
        <textarea
          id="critic-song-content"
          rows={10}
          placeholder="Napisi detaljnu kritiku pesme."
          value={critiqueText}
