...
export const songReviewInputSchema = z.object({
  ...
  songSpotifyId: z.string().length(22),
  rating10: z.number().int().min(1).max(10),
  liked: z.boolean(),
  description: z.string().max(4000),
});

export const criticSongReviewInputSchema = z.object({
  songId: z.string().min(1),
  songSpotifyId: z.string().length(22),
  title: z.string().trim().min(1, "Naslov je obavezan").max(128),
  rating10: z.number().int().min(1).max(10),
...
export type CriticSongReviewInput = z.infer<typeof criticSongReviewInputSchema>;
